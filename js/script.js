...
let simSpeed = 1;
// Master volume multiplier for all sound effects
...
// 按顺序显示的文字烟花内容
const randomWords = ["生日快乐！", "天天开心", "顺遂无虞", "皆得所愿"];
let wordIndex = 0;
const wordDotsMap = {};
randomWords.forEach((word) => {
        wordDotsMap[word] = MyMath.literalLattice(word, 3, "KaiTi, KaiTi_GB2312, STKaiti, 华文楷体, serif", "90px");
});

// 结束控制
let sequenceFinished = false;   // 4 句话放完即置 true
let endMessageShown  = false;   // 只弹一次结束语
...
// 随机获取一段文字
// Get the next phrase in sequence. If `advance` is false, the index will not
// increment so the same word can be reused without disrupting the main order.
function randomWord(advance = true) {
        // 如果四句都已触发，就标记结束并返回空串
...
// How long each word-shaped burst remains visible (ms)
const WORD_DISPLAY_TIME = 3000;
const wordShell = (wordText, size = 1) => {
        const config = crysanthemumShell(size);
        config.starLife = WORD_DISPLAY_TIME;
        config.starLifeVariation = 0;
        config.wordText = wordText;
        return config;
};
       ...
       if (nextWord) {
               ...
               for (let i = 0; i < count; i++) {
                       ...
                       const shell = new Shell({
                               ...shellFromConfig(size.size),
                               disableWord: true,
                       });
                       const delay = Math.random() * 300;
                       setTimeout(() => shell.launch(size.x, size.height), delay);
               }

               const wordDelay = 1000;
               const wordConfig = wordShell(nextWord, shellSizeSelector());
               const wordInstance = new Shell(wordConfig);
               setTimeout(() => wordInstance.launch(0.5, 0.5), wordDelay);

               const totalDelay = wordDelay + wordInstance.starLife + 800;
               return totalDelay / 1.25;
       }
	...
	if (sequenceFinished) {
		...
		if (!endMessageShown) {
			...
			COLOR_CODES_W_INVIS.forEach((color) => {
				totalActiveParticles += Star.active[color].length;
				totalActiveParticles += Spark.active[color].length;
			});
...
function getWordDots(word) {
	...
	var res = MyMath.literalLattice(word, 3, "KaiTi, KaiTi_GB2312, STKaiti, 华文楷体, serif", fontSize + "px");
...
class Shell {
        constructor(options) {
                ...
                this.wordText = options.wordText || null;
	...
	burst(x, y) {
		...
		//点阵星星工厂
                const dotStarFactory = (point, color, strobe, strobeColor) => {
                        ...
                        if (this.wordText) {
                                Star.add(point.x, point.y, color, 0, 0, this.starLife, 0, 0, 2, true);
                                return;
                        }
		...
		if (typeof this.color === "string") {
			...
			throw new Error("无效的烟花颜色。应为字符串或字符串数组，但得到:" + this.color);
                ...
                if (this.wordText) {
                        createWordBurst(this.wordText, dotStarFactory, x, y);
                } else if (!this.disableWord && store.state.config.wordShell) {
                        if (Math.random() < 0.1) {
                                if (Math.random() < 0.5) {
                                        // Don't advance the main sequence index when showing
                                        // random words from regular shells.
                                        createWordBurst(randomWord(false), dotStarFactory, x, y);
                                }
                        }
                }
...
const Star = {
	...
	_new() {
        ...
        add(x, y, color, angle, speed, life, speedOffX, speedOffY, size = 3, noGravity = false) {
		const instance = this._pool.pop() || this._new();
...
const Spark = {
	...
	_new() {
        ...
        add(x, y, color, angle, speed, life, noGravity = false) {
		const instance = this._pool.pop() || this._new();
...
const soundManager = {
	...
	playSound(type, scale = 1) {
		...
		// Volume descreases with scale.
                const scaledVolume = initialVolume * scale * MASTER_VOLUME;
...
function endFireworks() {
        ...
        overlay.className = "ignite-overlay";
        const text = document.createElement("div");
        text.className = "ignite-text";
        text.textContent = "表演结束，感谢观看";
...
if (IS_HEADER) {
        ...
        const overlay = document.getElementById("igniteOverlay");
        if (overlay) {
